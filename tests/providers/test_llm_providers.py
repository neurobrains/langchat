...
def test_openai_provider_creation():
    ...
    provider = OpenAI(api_key="test-key", model="gpt-4o-mini", temperature=0.7)
...
def test_openai_provider_requires_api_key():
    ...
    with pytest.raises(ValueError, match="At least one OpenAI API key is required"):
        OpenAI(api_key=None, api_keys=None)


def test_gemini_provider_creation():
    """Test Gemini provider initialization."""
    provider = Gemini(api_key="test-key", model="gemini-1.5-flash", temperature=1.0)
...
def test_anthropic_provider_creation():
    ...
    provider = Anthropic(api_key="test-key", model="claude-3-5-sonnet-20241022", temperature=1.0)
...
def test_ollama_provider_creation():
    ...
    provider = Ollama(model="llama2", base_url="http://localhost:11434", temperature=0.7)
...
def test_cohere_provider_creation():
    ...
    provider = Cohere(api_key="test-key", model="command", temperature=0.7)
...
def test_mistral_provider_creation():
    ...
    provider = Mistral(api_key="test-key", model="mistral-small-latest", temperature=0.7)
